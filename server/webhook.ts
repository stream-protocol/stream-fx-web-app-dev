...
export const sendNewRegistration = async (user: SavedUser, headers: any) => {
  ...
  axios.post(WEBHOOK_URL, {
    content: "",
    embeds: [{
      url: `https://streampayment.app/${user.link}`,
      title: "New registration",
      color: 38655,
      fields: [{
        name: `streampayment.app/${user.link}`,
      ...
      image: {
        url: `https://streampayment.app/random/gradient?nonce=${Date.now()}`
      }
    }]
  })
...
export const sendNewInvoice = async (invoice: Invoice, headers: any) => {
  ...
  axios.post(WEBHOOK_URL, {
    content: "",
    embeds: [{
      url: `https://streampayment.app/invoice/${invoice.invoiceNumber}`,
      title: "New Invoice",
      color: 38655,
      fields: [{
        name: `streampayment.app/${invoice.link}`,
    ...
    WEBHOOKS.forEach((url) => {
      axios.post(url, {
        ...
        embeds: [{
          ...
          color: 38655,
          fields: [{
            name: `streampayment.app/${transaction.link}`,
