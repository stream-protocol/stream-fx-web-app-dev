...
export const buyDomain = async (
  ...
  if (!bonk) {
    ...
    const _signature = await sendMoney(streampaymentPublicKey, cost, false);
    if (_signature) signature = _signature; else return;
  } else {
    const streamToBurn = plan === 1 ? 10**5 : 10**6;
    const _signature = await burnSPL('DezXAZ8z7PnrnRJjz3wXBoRgixCa6xjnB7YaB1pPB263', streamToBurn, 5);
    if (_signature) signature = _signature; else return;
  }
