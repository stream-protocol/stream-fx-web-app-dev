...
export function generateQRCodeURL (props: Props) {
  const amount = new BigNumber(props.amount);
  const data: any = props;
  data.amount = amount.toFixed(6);
  let fURL = `solana:${data.recipient}?amount=${data.amount}&`
  delete data.recipient;
  delete data.amount;
  if (data.splToken) {
    fURL += `spl-token=${data.splToken}&`;
  }
  delete data.splToken;
  const url = Object.keys(data).map((key) => `${key}=${encodeURIComponent(props[key as keyof Props] as any)}`).join('&');
  return fURL + url;
}
...
export async function findTransactionSignature(
  ...
  if (!length) throw new Error('not found');
...
export const getTxId = async (reference: string) => {
  return new Promise((resolve, reject) => {
    console.log('Checking for transaction...');
    const interval = setInterval(async () => {
      const RPC_URL = 'https://solana-mainnet.g.alchemy.com/v2/ZWDnDI0R0v6eHFg9RBXRbU52kB35QKWt';
      try {
        ...
        if (error.message.includes('not')) return;
        console.log(error);
      }
    }, 250);
  });
