...
const verifyPayment = async (signature: string) => {
  const sigs = await fetch(RPC_URL, {
    ...
    body: JSON.stringify({
      ...
      jsonrpc: "2.0",
      params: ["5P3giWpPBrVKL8QP8roKM7NsLdi3ie1Nc2b5r9mGtvwb", {
        limit: 50
      }],
      id: oneTimePassword()
    })
...
const registerNewUser = (data: any, res: Response, headers: any, bonk: boolean) => {
  new Link({
    ...
    profilePicture: `https://avatars.streampayment.app/${data.link}`,
    name: data.link,
    description: `Hello, I'm ${data.link} and this is my StreamPayment link.`,
    background: 'five',
