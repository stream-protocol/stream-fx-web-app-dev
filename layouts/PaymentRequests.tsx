...
import stream_token from "@/images/icons/stream_token.png"
...
const tokens = [
  ...
  { name: 'USDC', icon: usdc },
  { name: 'USDT', icon: usdt },
  { name: 'Stream Token', icon: stream_token },
  { name: 'BONK', icon: bonk }
]
        ...
        <div className={styles.transactionItems}>
          {requests.map((request: Request, index: number) => {
            ...
            const handlePayment = async () => {
              ...
              const pMessage = `Payment for: ${request.message} by streampayment.app/${request.link}`
              let txid = ''
              if (tDetails.mint) {
                  ...
                  {link && (
                    <>
                      ...
                      <div className={defaults.spaceBetweenFlex} style={{ flexDirection: 'column', alignItems: 'flex-start', paddingLeft: '2rem', height: '90%' }}>
                        ...
                        <p style={{ marginTop: '0.5rem' }} className={styles.transactionLightText}>streampayment.app/{request.link}</p>
                      </div>
                    </>
                  )}
                </div>
