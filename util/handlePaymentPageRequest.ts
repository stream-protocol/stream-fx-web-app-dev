...
export const handlePaymentPageRequest = async (query: any) => {
  ...
  if (reserved) {
    ...
    const baseURL = process.env.NODE_ENV === 'development' ? 'http://localhost:3000' : 'https://streampayment.app'
  ...
  if (query.pay.includes('.sol')) {
    ...
    if (address) {
      const solProps = {
        ...
        link: query.pay,
        publicKey: address,
        profilePicture: `https://avatars.streampayment.app/${query.pay}`,
        description: `Hi, I'm ${query.pay} and this is my StreamPayment link.`,
        background: 'one',
        isConfirmed: true,
        price: data.ok ? price : 0,
      }
