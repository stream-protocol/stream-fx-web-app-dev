...
export const DashboardLink = ({ link, price }: any) => {
  ...
  const handleClick = async () => {
    if (disabled) return
    const streampaymentPublicKey = new PublicKey('5P3giWpPBrVKL8QP8roKM7NsLdi3ie1Nc2b5r9mGtvwb')
    const nineDollars = 9 / price
    const signature = await sendMoney(streampaymentPublicKey, nineDollars, true)
    if (!signature) return
    setDisabled(true)
          ...
          <div className={defaults.spaceBetweenFlex}>
            <div className={defaults.spaceBetweenFlex} style={{ width: 'max-content'}}>
              ...
              <div style={{ marginLeft: '2rem' }}>
                ...
                <p style={{ fontSize: '1rem', marginBottom: '0rem', marginTop: '0.5rem' }} className={dashStyles.dashboardSubHeading}>streampayment.app/{link.link}</p>
              </div>
            </div>
