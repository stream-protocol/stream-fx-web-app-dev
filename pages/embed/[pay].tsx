...
export default function Pay(props: Props) {
  ...
  <>
    <DefaultHead
    image={notRegistered || showNotFound ? 'https://streampayment.app/meta.png' : `https://wp.streampayment.app/${props.name}/${props.link}/${pfpEncodedLink}`}
    title={notRegistered || showNotFound ? 'StreamPayment - Empowering Seamless Crypto Transactions for Everyone' : `Pay - ${props.name}`}
    />
    ...
    <>
      ...
      {user.plan === 1 && (
        ...
        <a className={styles.getLink}>
          ...
          {(linkOwner && !props.link.includes('.sol')) ? 'Edit this link' : ' Your StreamPayment Link Now'}
