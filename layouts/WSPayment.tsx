...
export const MainScreen = ({
  ...
  return (
    <>
      ...
      >
        <DefaultHead
          image={notRegistered || showNotFound ? 'https://streampayment.app/meta.png' : `https://streampayment.app/${props.name}/${props.link}/${pfpEncodedLink}`}
          title={notRegistered || showNotFound ? 'StreamPayment™: Simplify, Secure, and Streamline Your Crypto Payments Experience' : `Pay - ${props.name}`}
        />
        <LeftPanel
          editLink={editLink}
...
export const RightPane = ({
  ...
  const tokens = ['solana', 'stream token', 'usdc', 'usdt', 'bonk'];
  const ipfsURL = props.profilePicture.split('https://')[1];
  const avatarURL = ipfsURL.includes('ipfs') ? `https://wp.streampayment.app/${ipfsURL}` : props.profilePicture;
  return (
    <div className={styles.rightPane}>
      <img
        ...
        onError={({ currentTarget }) => {
          ...
          currentTarget.src=`https://avatars.streampayment.app/${props.link}`;
        }}
      />
    ...
    <div className={styles.rightPane}>
      ...
      {notRegistered && (
        <div>
          ...
          <p className={styles.lightText}>
            Claim Your Piece of the Web: Secure Your Link Now!. Register This Link from Just $1 and Make It Yours Toda
          </p>
          <button
      ...
      {reserved && (
        ...
        >
          ...
          />
          <div>
            <p className={styles.bigDarkText}>Already Taken or Reserved</p>
            <p className={styles.lightText}>Already Taken or Reserved</p>
            <button
              onClick={() => router.push('/')}
