...
export default function Embed({ setUser, user }: { setUser: any, user: string }) {
      ...
      if (publicKey) {
        ...
        .then((res) => {
          ...
          if (proLinks.length > 0) {
            ...
            setError("Enhance Your Access with Premium Links");
          }
        })
  ...
  return (
    <>
    <DefaultHead title="Embed StreamPayment" />
    ...
    <main className={dashStyles.main} style={{ overflowX: 'auto' }}>
      ...
      {!error && !loading && links[0] && <EmbedLink links={links} />}
    </main>
    </>
  );
}